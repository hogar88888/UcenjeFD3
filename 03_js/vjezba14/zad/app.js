...
function osvjeziPrikaz() {
    ...
    podaciStranice.forEach(korisnik => {
        ...
        let pocetak = performance.now();
        /*
        
        red.innerHTML = `
            <td class="sifra">${korisnik.id}</td>
        ...
        */
        ...
        red.insertAdjacentHTML('afterbegin',`
            <td class="sifra">${korisnik.id}</td>
            <td><span class="ime">${korisnik.ime}</span></td>
            <td><span class="ime">${korisnik.prezime}</span></td>
            <td>${datumHr}</td>
       ...
       // kreirajCelijeDOM(red,korisnik); // putem DOM, spriječava XSS (https://developer.mozilla.org/en-US/docs/Web/Security/Attacks/XSS)
        ukupnoVrijeme += performance.now() - pocetak;
...
document.getElementById('poljePretraga').addEventListener('input', (e) => {
    ...
    (k.prezime.toLowerCase() + ' ' + k.ime.toLowerCase()).includes(pojam));
    trenutnaStranica = 1; 
    osvjeziPrikaz();
});
...
function izracunajStatistiku() {
    ...
    ispis.innerHTML = `
        ...
        Prosjek: ${(filtriraniPodaci.length > 2 ? sumaBodova / filtriraniPodaci.length : 0).toFixed(2)}<br>
